...
FOUND=""
INSTALLED=""
uninstallpath="/etc/webiopi/uninstall"
if [ "$#" = "1" ]; then
	command="$1"
else
	command="none"
...
for python in $SEARCH; do
	...
	if [ -x $program ]; then
		...
		if [ -f "$include/Python.h" ]; then
			...
			$python setup.py install --record "${uninstallpath}/installed_components"
			if [ "$?" -ne "0" ]; then
...
# Copy Weaved enablement file
cp python/weavedconnectd.conf /etc/webiopi/weavedconnectd.conf
...
#Add uninstaller
cp -rf uninstall.sh "${uninstallpath}/uninstall.sh"
chmod 0755 "${uninstallpath}/uninstall.sh"
