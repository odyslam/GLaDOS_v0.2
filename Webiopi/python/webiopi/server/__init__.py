...
from webiopi.utils import config as confmanager
...
OLD_CONFIG_FILE = "/etc/webiopi/config.devices"
...
class Server():
    def __init__(self, port=8000, coap_port=5683, login=None, password=None, passwdfile=None, configfile=None, scriptfile=None):
        ...
        config = confmanager.loadConfig(configfile)
        devicesConfig = confmanager.Config(OLD_CONFIG_FILE)
        ...
        manager.addDeviceConf(devices, "manual")
        
        if os.path.isfile(OLD_CONFIG_FILE):
            devices = devicesConfig.items("DEVICES")
            manager.addDeviceConf(devices, "rest")
