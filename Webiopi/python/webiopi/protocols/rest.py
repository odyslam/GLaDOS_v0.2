    ...
    def do_POST(self, relativePath, data, compact=False):
        ...
        if relativePath.startswith("GPIO/"):
            ...
            if len(path) > 0:
                ...
                return manager.addDeviceJSON(json)

        else: # path unknowns
            return (0, None, None)
        
    
    def do_DELETE(self, relativePath, compact=False):
        if relativePath.startswith("devices/"):
            name = relativePath.replace("devices/", "")
            if '/' in name:
                name = name.split('/')[0]
            return manager.removeDevice(name)
            
        return (404, None, None)

    def do_PUT(self, relativePath, data, compact=False):
        if relativePath.startswith("devices/"):
            name = relativePath.replace("devices/", "")
            if '/' in name:
                name = name.split('/')[0]
            json = JSON.loads(data.decode())
            return manager.updateDevice(name, json)
