...
class HTTPHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def checkAuthentication(self):
        #IP address format for translation: ::ffff:192.168.0.104
        client_address=str(self.client_address[0])
        if client_address.endswith('localhost') or client_address.endswith('127.0.0.1') or client_address.endswith('0:0:0:0:0:0:0:1') or client_address.endswith('::1'):
            return True
    ...
    def processRequest(self):
        ...
        try:
            ...
            if self.command == "GET":
            ...
            elif self.command == "POST" or self.command == "PUT":
                length = 0
                ...
                if self.command == "POST":
                    ...
                    result = self.server.handler.do_PUT(relativePath, self.rfile.read(length), compact)
                    
            elif self.command == "DELETE":
                result = self.server.handler.do_DELETE(relativePath, compact)
            else:
